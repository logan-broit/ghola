...
const markSupersedes = `SELECT pg_ghola.mark_supersedes($1, $2)`

const recallQuery = `
SELECT (r).mneme_id, (r).score, (r).content_match, (r).activation,
       (r).hebbian_boost, (r).confidence, (r).concept, (r).content
FROM pg_ghola.recall($1, $2, $3::vector, $4, $5, $6::pg_ghola.score_weights, $7, $8, $9, $10) AS r
`

const confirmRecall = `SELECT pg_ghola.confirm_recall($1::uuid[])`

const deleteMneme = `
DELETE FROM pg_ghola.mnemes
...
const weightedConfirmSingle = `UPDATE pg_ghola.mnemes SET confidence = pg_ghola.bayesian_update(confidence, $2) WHERE id = $1`

const feedbackUpdate = `UPDATE pg_ghola.mnemes SET confidence = pg_ghola.bayesian_update(confidence, $2) WHERE id = $1 AND workspace_id = ANY($3) RETURNING confidence`
