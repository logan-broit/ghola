// pg_recall::associations — Typed association management
//
...
fn mark_supersedes(newer_id: pgrx::Uuid, older_id: pgrx::Uuid) -> &'static str {
    ...
    Spi::connect_mut(|client| {
        ...
        client
            .update(
                &format!(
                    "UPDATE pg_recall.mnemes SET state = 'archived' \
...
fn mark_supports(supporting_id: pgrx::Uuid, supported_id: pgrx::Uuid) -> &'static str {
    ...
    Spi::connect_mut(|client| {
        ...
        let prior_row = client
            .select(
                &format!(
                    "SELECT confidence, tier FROM pg_recall.mnemes \
        ...
        if let Some(r) = prior_row.into_iter().next() {
            ...
            client
                .update(
                    &format!(
                        "UPDATE pg_recall.mnemes SET confidence = {clamped} \
...
mod tests {
    ...
    fn test_mark_supersedes_archives_older() {
        ...
        Spi::run(&format!(
            "SELECT pg_recall.mark_supersedes('{m2}'::uuid, '{m1}'::uuid)"
        ))
        .expect("mark_supersedes failed");

        // Older mneme should be archived
        let state = Spi::get_one::<String>(&format!(
            "SELECT state FROM pg_recall.mnemes WHERE id = '{m1}'::uuid"
        ))
        .expect("query failed")
        .expect("null");
    ...
    fn test_mark_supersedes_self_panics() {
        ...
        Spi::run(&format!(
            "SELECT pg_recall.mark_supersedes('{m1}'::uuid, '{m1}'::uuid)"
        ))
        .expect("should panic");
    }
    ...
    fn test_mark_supports_boosts_confidence() {
        ...
        Spi::run(&format!(
            "SELECT pg_recall.mark_supports('{m1}'::uuid, '{m2}'::uuid)"
        ))
    ...
    fn test_get_typed_associations_all() {
        ...
        Spi::run(&format!(
            "SELECT pg_recall.mark_supersedes('{m3}'::uuid, '{m2}'::uuid)"
        )).expect("supersedes failed");

        // Get all associations for m2
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM pg_recall.get_typed_associations('{m2}'::uuid)"
        ))
        .expect("query failed")
        .expect("null");
    ...
    fn test_get_typed_associations_filtered() {
        ...
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM pg_recall.get_typed_associations('{m2}'::uuid, 'supports')"
        ))
        .expect("query failed")
        .expect("null");
