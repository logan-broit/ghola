// pg_recall::contradiction — Contradiction detection and flagging
//
            ...
            .select(
                &format!(
                    "SELECT workspace_id::text, embedding::text, concept, content \
                     FROM pg_recall.mnemes WHERE id = '{mneme_id}'"
                ),
                None,
                &[],
            )
        ...
        for candidate in &to_flag {
            ...
            let inserted = client
                .select(
                    &format!(
                        "WITH ins AS ( \
                             INSERT INTO pg_recall.contradiction_candidates \
    ...
    Spi::connect_mut(|client| {
            ...
            .select(
                &format!(
                    "SELECT mneme_a::text, mneme_b::text, status \
                     FROM pg_recall.contradiction_candidates \
        ...
        if resolution == "confirmed" {
            ...
            let prior_row = client
                .select(
                    &format!(
                        "SELECT confidence FROM pg_recall.mnemes \
            ...
            if let Some(r) = prior_row.into_iter().next() {
                ...
                client
                    .update(
                        &format!(
                            "UPDATE pg_recall.mnemes SET confidence = {posterior} \
        ...
        client
            .update(
                &format!(
                    "UPDATE pg_recall.contradiction_candidates \
            ...
            .select(
                &format!(
                    "SELECT cc.id, cc.similarity, cc.concept_overlap, \
                         ...
                         cc.created_at \
                     FROM pg_recall.contradiction_candidates cc \
                     JOIN pg_recall.mnemes a ON cc.mneme_a = a.id \
                     JOIN pg_recall.mnemes b ON cc.mneme_b = b.id \
...
fn scan_workspace_contradictions(
    ...
    for id_str in &mneme_ids {
        let flagged = Spi::get_one::<i64>(&format!(
            "SELECT pg_recall.flag_contradictions('{id_str}'::uuid, {similarity_threshold})"
        ))
        .unwrap_or(Some(0))
        .unwrap_or(0);
...
mod tests {
    ...
    fn test_contradiction_candidates_table_exists() {
        let count = Spi::get_one::<i64>(
            "SELECT count(*) FROM information_schema.tables \
             WHERE table_schema = 'pg_recall' AND table_name = 'contradiction_candidates'",
        )
        .expect("query failed")
        .expect("null");
        assert_eq!(count, 1);
        ...
        for type_name in &["contradiction_candidate_result", "contradiction_detail"] {
            let exists = Spi::get_one::<bool>(&format!(
                "SELECT EXISTS(
                    ...
                    WHERE n.nspname = 'pg_recall' AND t.typname = '{type_name}'
                )"
            ))
            .unwrap()
            .unwrap();
    ...
    fn test_check_contradictions_finds_similar() {
        ...
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM pg_recall.check_contradictions('{m2}'::uuid, 0.85)"
        ))
        .expect("query failed")
        .expect("null");
    ...
    fn test_flag_contradictions_inserts_candidates() {
        ...
        let count = Spi::get_one::<i64>(
            "SELECT count(*) FROM pg_recall.contradiction_candidates WHERE status = 'pending'",
        )
        .expect("query failed")
        .expect("null");
        assert!(count >= 1);
    ...
    fn test_flag_contradictions_concept_overlap() {
        ...
        let overlap = Spi::get_one::<bool>(
            "SELECT concept_overlap FROM pg_recall.contradiction_candidates LIMIT 1",
        )
        .expect("query failed")
        .expect("null");
    ...
    fn test_resolve_confirmed_penalizes_newer() {
        ...
        let candidate_id = Spi::get_one::<i64>(
            "SELECT id FROM pg_recall.contradiction_candidates LIMIT 1",
        )
        .expect("query failed")
        .expect("null");
    ...
    fn test_dormant_excluded_from_detection() {
        ...
        // Mark m1 as dormant
        Spi::run(&format!(
            "UPDATE pg_recall.mnemes SET state = 'dormant' WHERE id = '{m1}'::uuid"
    ...
    fn test_get_pending_contradictions() {
        ...
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM pg_recall.get_pending_contradictions('{ws}'::uuid, 50)"
        ))
        .expect("query failed")
        .expect("null");
    ...
    fn test_cascade_delete_cleans_candidates() {
        ...
        Spi::run(&format!(
            "DELETE FROM pg_recall.mnemes WHERE id = '{m2}'::uuid"
    ...
    fn test_resolve_invalid_resolution() {
        Spi::run(
            "SELECT pg_recall.resolve_contradiction(1, 'invalid')"
    ...
    fn test_resolve_nonexistent_candidate() {
        Spi::run(
            "SELECT pg_recall.resolve_contradiction(99999, 'confirmed')"
        ).expect("should have failed");
    }
}
