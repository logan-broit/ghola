...
/// then inserts a single row into ghola.co_activation_queue.
#[pg_extern]
    ...
    let results = Spi::connect(|client| {
        let query = format!(
            "SELECT related_id, weight FROM ( \
                SELECT dst_id AS related_id, weight FROM ghola.associations \
                WHERE src_id = '{mneme_id}' AND weight >= {min_weight} \
                UNION ALL \
                SELECT src_id AS related_id, weight FROM ghola.associations \
...
fn update_confidence(mneme_id: pgrx::Uuid, evidence: f64) -> f64 {
    Spi::connect_mut(|client| {
        ...
        let tup_table = client
            .select(
                &format!(
                    "SELECT confidence, tier FROM ghola.mnemes WHERE id = '{mneme_id}' FOR UPDATE"
                ),
                None,
                &[],
            )
        ...
        match row {
            ...
            Some(r) => {
                ...
                client
                    .update(
                        &format!(
                            "UPDATE ghola.mnemes SET confidence = {clamped} WHERE id = '{mneme_id}'"
                        ),
                        None,
                        &[],
                    )
    ...
    for id in &mneme_ids {
        Spi::connect_mut(|client| {
            let tup_table = client
                .select(
                    &format!(
                        "SELECT confidence, tier FROM ghola.mnemes WHERE id = '{id}' FOR UPDATE"
                    ),
                    None,
                    &[],
                )
            ...
            match row {
                ...
                Some(r) => {
                    ...
                    client
                        .update(
                            &format!(
                                "UPDATE ghola.mnemes SET confidence = {clamped} WHERE id = '{id}'"
                            ),
                            None,
                            &[],
                        )
...
fn process_co_activation_batch(batch_limit: default!(i32, 100)) -> i64 {
    ...
    Spi::connect_mut(|client| {
        ...
        let rows = client
            .select(
                &format!(
                    "SELECT id, mneme_ids, scores FROM ghola.co_activation_queue \
        ...
        for ((src, dst), signal) in &pair_signals {
            client
                .update(
                    &format!(
                        "INSERT INTO ghola.associations (src_id, dst_id, association_type, weight, co_activations, updated_at) \
                         VALUES ('{src}', '{dst}', 'hebbian', \
                             LEAST(1.0, EXP(LN(0.01) + {signal} * LN(1.01))), \
                             1, now()) \
                         ON CONFLICT (src_id, dst_id, association_type) DO UPDATE SET \
                             weight = LEAST(1.0, EXP(LN(ghola.associations.weight) + {signal} * LN(1.01))), \
                             co_activations = ghola.associations.co_activations + 1, \
                             updated_at = now()"
                    ),
                    None,
                    &[],
        ...
        for mid in &all_mneme_ids {
            client
                .update(
                    &format!(
                        "UPDATE ghola.mnemes SET access_count = access_count + 1, \
        ...
        for qid in &consumed_ids {
            client
                .update(
                    &format!("DELETE FROM ghola.co_activation_queue WHERE id = {qid}"),
                    None,
                    &[],
                )
...
mod tests {
    ...
    fn test_record_co_activation_inserts() {
        ...
        let count = Spi::get_one::<i64>("SELECT count(*) FROM ghola.co_activation_queue")
            .expect("count failed")
            .expect("null count");
    ...
    fn test_get_associations_empty() {
        ...
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM ghola.get_associations('{m1}'::uuid, 0.01)"
        ))
        .expect("query failed")
        .expect("null count");
    ...
    fn test_get_associations_min_weight_filter() {
        ...
        let count = Spi::get_one::<i64>(&format!(
            "SELECT count(*) FROM ghola.get_associations('{m1}'::uuid, 0.3)"
        ))
        .expect("query failed")
        .expect("null count");
    ...
    fn test_get_associations_ordered_by_weight_desc() {
        ...
        let first_weight = Spi::get_one::<f64>(&format!(
            "SELECT weight FROM ghola.get_associations('{m1}'::uuid, 0.01) LIMIT 1"
        ))
        .expect("query failed")
        .expect("null weight");
    ...
    fn test_update_confidence_strong_evidence() {
        ...
        let new_conf = Spi::get_one::<f64>(&format!(
            "SELECT ghola.update_confidence('{m1}'::uuid, 0.95)"
        ))
        .expect("query failed")
        .expect("null result");
    ...
    fn test_update_confidence_contradiction() {
        ...
        Spi::run(&format!(
            "UPDATE ghola.mnemes SET confidence = 0.8 WHERE id = '{m1}'::uuid"
        ))
        .expect("update failed");

        let new_conf = Spi::get_one::<f64>(&format!(
            "SELECT ghola.update_confidence('{m1}'::uuid, 0.10)"
        ))
        .expect("query failed")
        .expect("null result");
    ...
    fn test_update_confidence_nonexistent() {
        ...
        Spi::get_one::<f64>(
            "SELECT ghola.update_confidence(\
    ...
    fn test_confirm_recall_updates_all() {
        ...
        Spi::run(&format!(
            "SELECT ghola.confirm_recall(ARRAY['{m1}','{m2}','{m3}']::uuid[])"
        ))
    ...
    fn test_core_tier_confidence_floor() {
        ...
        Spi::run(&format!(
            "UPDATE ghola.mnemes SET tier = 'core', confidence = 0.5 WHERE id = '{m1}'::uuid"
        ))
        .expect("update failed");
    ...
    fn test_process_batch_updates_access_tracking() {
        ...
        Spi::run("SELECT ghola.process_co_activation_batch(100)")
    ...
    fn test_process_batch_new_association_seeded() {
        ...
        let weight = Spi::get_one::<f64>("SELECT weight FROM ghola.associations LIMIT 1")
            .expect("query failed")
            .expect("null weight");
    ...
    fn test_weight_capped_at_one() {
        ...
        Spi::run(&format!(
            "INSERT INTO ghola.associations (src_id, dst_id, weight, co_activations) \
    ...
    fn test_co_activations_counter_increments() {
        ...
        let co_acts = Spi::get_one::<i32>(
            "SELECT co_activations FROM ghola.associations LIMIT 1",
        )
        .expect("query failed")
        .expect("null");
    ...
    fn test_state_tier_excluded_from_hebbian() {
        ...
        Spi::run(&format!(
            "UPDATE ghola.mnemes SET tier = 'state' WHERE id = '{m2}'::uuid"
        ))
        .expect("update failed");
