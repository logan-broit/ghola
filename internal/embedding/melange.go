// Package embedding implements core.Embedder against the Melange
...
// Melange is the embedding client. Retries 5xx responses with
...
// New builds a Melange client. `baseURL` points at the service root
// (e.g. "http://localhost:8082"); model is the name the server uses
// (e.g. "qwen3-embedding").
func New(baseURL, model string) *Melange {
	return &Melange{
...
func (m *Melange) WithHTTPClient(h *http.Client) *Melange {
	m.http = h
	return m
}

// WithRetries overrides the default retry budget (3).
func (m *Melange) WithRetries(n int) *Melange {
	m.retries = n
	return m
}
...
func (m *Melange) Embed(ctx context.Context, text string) ([]float32, error) {
