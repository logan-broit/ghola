...
//	  "cwd":"/home/loganb/ai","gitBranch":"main",
...
func (a *Adapter) Walk(root string) iter.Seq[importlogs.SessionFile] {
	return func(yield func(importlogs.SessionFile) bool) {
		_ = filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
			if err != nil {
				a.logger.Debug("walk error", "path", path, "err", err.Error())
				return nil
			}
			if d.IsDir() {
				return nil
			...
			info, err := d.Info()
			if err != nil {
				a.logger.Debug("stat error", "path", path, "err", err.Error())
				return nil
			}
...
func (a *Adapter) Parse(sf importlogs.SessionFile) (*importlogs.NormalizedSession, error) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &ev); err != nil {
			a.logger.Debug("json parse error", "path", sf.Path, "err", err.Error())
			continue
		}
		if ev.SessionID != "" {
