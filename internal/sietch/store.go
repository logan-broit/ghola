...
func (s *Store) OpenSession(ctx context.Context, sess core.Session) error {
	...
	_, err = db.ExecContext(ctx, `
		INSERT INTO session (
			id, user_id, started_at, ended_at, cwd, git_branch,
			agent_kind, source_device
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *Store) readSessionRow(ctx context.Context, sessionID string) (core.Session, error) {
	...
	var (
		...
		eventCount                int
		summary, cwd, gitBranch, agentKind, sourceDevice sql.NullString
	)
	err = db.QueryRowContext(ctx, `
		SELECT id, user_id, started_at, ended_at, last_event_at, event_count,
		       summary, cwd, git_branch, agent_kind, source_device
		FROM session WHERE id = ?
	`, sessionID).Scan(&id, &uid, &started, &ended, &lastEvent, &eventCount,
		&summary, &cwd, &gitBranch, &agentKind, &sourceDevice)
	if err != nil {
		return core.Session{}, err
	}
	sess := core.Session{
